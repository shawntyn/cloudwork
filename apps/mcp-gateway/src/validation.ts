...
const text = z.string().trim().min(1).max(100).regex(/^[^\x00-\x1f\x7f]+$/);
...
export const connectionInput = z.object({ name: text, url: z.string().trim().min(1).max(2048), authType: z.enum(['none', 'bearer', 'headers']), token: credential.optional(), headers: headers.optional(), enabled: z.boolean().optional() }).strict();
export const connectionPatch = connectionInput.partial().refine(value => Object.keys(value).length > 0, 'Provide a change');
