...
export function McpSettings() {
    ...
    return <div className="dashboard mcp-page cw-shell"><AppSidebar active="settings"/><div className="cw-shell-main">
        <Header user={user}><span className="header-divider"/><span className="header-section">{tr("MCP 连接", "MCP connections")}</span></Header>
        <main className="dashboard-main mcp-main">
            ...
            <div className="dashboard-intro">
                <div><div className="eyebrow"> {tr("连接工具", "CONNECTED TOOLS")}</div><h1>{tr("MCP 连接", "MCP connections")}<span className="heading-dot">.</span></h1><p className="muted"> {tr("连接 MCP 服务器，再选择哪些工作区可以使用它的工具。", "Connect an MCP server, then choose which workspaces can use its tools.")}</p></div>
            ...
            <section className="mcp-connections" aria-label={tr("MCP 连接", "MCP connections")} aria-busy={loading}>
                {loading ? <div className="empty-state"><span className="spinner"/><p>{tr("正在加载连接…", "Loading connections…")}</p></div> : !loadError && connections.length === 0 ? <div className="empty-state mcp-empty"><span className="empty-icon"><Icon name="plug" size={30}/></span><h2>{tr("连接你的工具", "Your tools, connected")}</h2><p>{tr("添加 HTTP MCP 服务器，让工作区 Agent 使用它的工具。", "Add an HTTP MCP server to make its tools available to your workspace agent.")}</p><button className="button button-secondary" disabled={origins.length === 0} onClick={() => setEditing("new")}><Icon name="plus"/>{tr("添加第一个连接", "Add your first connection")}</button></div> : connections.map(connection => <article className="mcp-card" key={connection.id}>
                    <div className="mcp-card-top"><div className="mcp-card-identity"><span className="workspace-icon color-0"><Icon name="plug" size={23}/></span><div><h2>{mcpConnectionLabel(connection)}</h2><p className="small muted">{connection.serverName || tr("MCP 服务器", "MCP server")}</p></div></div><button className={`mcp-enable ${connection.enabled ? "is-enabled" : ""}`} role="switch" aria-checked={connection.enabled} aria-label={tr(`启用 ${mcpConnectionLabel(connection)}`, `Enable ${mcpConnectionLabel(connection)}`)} disabled={!!busy} onClick={() => { void action(connection, "toggle"); }}><span className="mcp-switch-track"><span/></span>{connection.enabled ? tr("已启用", "Enabled") : tr("已停用", "Disabled")}</button></div>
