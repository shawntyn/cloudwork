...
test('real SDK loads per-run MCP tools, calls the gateway, isolates concurrent snapshots and removes overlays', { timeout: 60_000 }, async () => {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (request.url?.startsWith('/mcp/')) {
        ...
        else if (body.method === 'tools/list') result = { tools: [{ name: `probe_${name}`, description: 'Local MCP fixture', inputSchema: { type: 'object', properties: {} } }] };
      ...
      if (marker === 'MCP_WARM' && !messages.slice(userIndex + 1).some(message => message.role === 'tool')) {
        send({ role: 'assistant', tool_calls: [{ index: 0, id: 'call_local_mcp', type: 'function', function: { name: 'mcp__alpha__probe_alpha', arguments: '{}' } }] });
        send({}, 'tool_calls');
  ...
  const snapshot = (name: string): McpRunSnapshot => ({ runId: `run_${name}`, revision: `revision_${name}`, connections: [{ id: `id_${name}`, serverName: name, url: `${baseUrl}/mcp/${name}`, token: `scoped-${name}-token` }] });
  ...
  try {
    ...
    const warm = await collect('sess_mcp_a', 'MCP_WARM', snapshot('alpha'));
    assert.equal(toolCalls, 1);
    assert.ok(warm.some(event => event.type === 'tool-start' && event.name === 'mcp__alpha__probe_alpha'));
    assert.ok(warm.some(event => event.type === 'tool-result' && JSON.stringify(event.output).includes('[redacted]')));
    assert.equal(JSON.stringify(warm).includes('scoped-alpha-token'), false);
    assert.deepEqual(await readdir(join(home, '.cloud-work', 'mcp-runs')), []);
    const concurrent = Promise.all([
      collect('sess_mcp_a', 'MCP_BETA', snapshot('beta')),
      collect('sess_mcp_b', 'MCP_GAMMA', snapshot('gamma')),
    ]);
    ...
    for (const overlay of overlays) assert.equal(/scoped-(alpha|beta|gamma)-token/.test(overlay), false);
    releaseResponses();
    await concurrent;
    await collect('sess_mcp_a', 'MCP_NONE');
    for (const [marker, name] of [['MCP_WARM', 'alpha'], ['MCP_BETA', 'beta'], ['MCP_GAMMA', 'gamma']]) {
      assert.deepEqual(catalogs.get(marker!)!.filter(tool => tool.startsWith('mcp__')), [`mcp__${name}__probe_${name}`]);
      assert.ok(authorizations.some(auth => auth.path === `/mcp/${name}` && auth.value === `Bearer scoped-${name}-token`));
    }
