...
import type { FileEntry } from '@cloud-work/protocol';
...
async function pinned<T>(root: string, parts: string[], fn: (parent: string) => Promise<T>): Promise<T> {
...
async function parent<T>(root: string, relative: string, fn: (target: string) => Promise<T>): Promise<T> {
  const parts = relativeParts(relative);
  const name = parts.pop()!;
  return pinned(root, parts, dir => fn(path.join(dir, name)));
}
export async function listFiles(root: string, relative = ''): Promise<FileEntry[]> {
  const parts = relativeParts(relative, true);
  return pinned(root, parts, async dir => {
...
export async function readFileContent(root: string, relative: string): Promise<string> {
  return parent(root, relative, async target => {
    const h = await open(target, constants.O_RDONLY | constants.O_NOFOLLOW | constants.O_NONBLOCK);
    try {
      const stat = await h.stat();
      if (!stat.isFile() || stat.size > MAX_FILE_BYTES) throw new Error('Only regular text files up to 2 MiB are supported');
      const buffer = Buffer.alloc(MAX_FILE_BYTES + 1);
      const { bytesRead } = await h.read(buffer, 0, buffer.length, 0);
      if (bytesRead > MAX_FILE_BYTES) throw new Error('File is too large');
      if (buffer.subarray(0, bytesRead).includes(0)) throw new Error('Binary files cannot be edited');
      return buffer.subarray(0, bytesRead).toString('utf8');
    } finally { await h.close(); }
  });
}
export async function writeFileContent(root: string, relative: string, content: string): Promise<void> {
  if (typeof content !== 'string' || Buffer.byteLength(content) > MAX_FILE_BYTES || content.includes('\0')) throw new Error('Text content exceeds 2 MiB or contains binary data');
  await parent(root, relative, async target => {
...
export async function makeDirectory(root: string, relative: string): Promise<void> {
  await parent(root, relative, async target => { await mkdir(target, { mode: 0o700 }); });
}
export async function renameEntry(root: string, from: string, to: string): Promise<void> {
  await parent(root, from, source => parent(root, to, async target => {
...
export async function deleteEntry(root: string, relative: string): Promise<void> {
  await parent(root, relative, async target => { await rm(target, { recursive: true, force: false }); });
}
